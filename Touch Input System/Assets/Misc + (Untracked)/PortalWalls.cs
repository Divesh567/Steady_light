    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Ball") || collision.CompareTag("Block") || collision.CompareTag("Star"))
        {
            ...
            if (_otherObjectName != collision.gameObject.name)
            {
                ...
                float _objectMagnitude = _otherObject.linearVelocity.magnitude;
                _otherObject.linearVelocity = _otherPortal.transform.up * _objectMagnitude;
        ...
        else if (collision.CompareTag("Spike"))
        {
            ...
            if (_otherObjectName != collision.gameObject.name)
            {
                ...
                {
                    float _objectMagnitude = collision.GetComponent<Rigidbody2D>().linearVelocity.magnitude;
                    collision.GetComponent<Rigidbody2D>().linearVelocity = _otherPortal.transform.up * _objectMagnitude;
                }
                collision.transform.position = _otherPortal.position;
                _otherPortal.GetComponent<PortalWalls>()._otherObjectName = collision.gameObject.name;
            }
